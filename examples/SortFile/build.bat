...
set "_SOURCE_RSRC_DIR=%_SOURCE_DIR%\main\resources"
...
:run
set "__INPUT_FILE=%_SOURCE_RSRC_DIR%\unsorted.txt"
if not exist "%__INPUT_FILE%" (
    echo %_ERROR_LABEL% Input file "%__INPUT_FILE%" not found 1>&2
    set _EXITCODE=1
    goto :eof
)
set "__OUTPUT_DIR=C:\Temp\"
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% copy "%__INPUT_FILE%" "%__OUTPUT_DIR%" 1>&2
) else if %_VERBOSE%==1 ( echo Copy input file to temporary directory "%__OUTPUT_DIR%" 1>&2
)
copy "%__INPUT_FILE%" "%__OUTPUT_DIR%" 1>NUL
