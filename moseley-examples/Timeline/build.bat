...
set "_TARGET_FILE=%_TARGET_DIR%\%_MAIN_NAME%.exe"
...
if %_DEBUG%==1 (
    ...
    echo %_DEBUG_LABEL% Variables  : "COBJ_HOME=%COBJ_HOME%" 1>&2
...
echo     %__BEG_O%run%__END%         execute COBOL program "!_TARGET_FILE:%_ROOT_DIR%=!"
goto :eof
...
call :action_required "%_TARGET_FILE%" "%_SOURCE_MAIN_DIR%\*.cbl"
...
set __SOURCE_FILES=
set __N=0
for /f "delims=" %%f in ('dir /s /b "%_SOURCE_MAIN_DIR%\*.cpy" "%_SOURCE_MAIN_DIR%\*.cbl" "%_SOURCE_MAIN_DIR%\*.cob" 2^>NUL') do (
...
set __COBC_OPTS=-std=%_STANDARD% -x -o "%_TARGET_FILE%" -I "%_SOURCE_MAIN_DIR%" -I "%MSYS_HOME%\mingw64\include"
...
set _CBLLINK_OPTS=-F -M%_MAIN_NAME% -O%_TARGET_FILE%
...
if not exist "%_TARGET_FILE%" (
    echo %_DEBUG_LABEL% Main program "!_TARGET_FILE:%_ROOT_DIR%=!" not found 1>&2
    set _EXITCODE=1
    goto :eof
)
set "__PATH=%PATH%"
...
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% Execute program "!_TARGET_FILE:%_ROOT_DIR%=!" 1>&2
) else if %_VERBOSE%==1 ( echo Execute program "!_TARGET_FILE:%_ROOT_DIR%=!" 1>&2
)
call "%_TARGET_FILE%"
if not %ERRORLEVEL%==0 (
    set "PATH=%__PATH%"
    echo %_DEBUG_LABEL% Failed to execute program "!_TARGET_FILE:%_ROOT_DIR%=!" 1>&2
    set _EXITCODE=1
    goto :eof
)
set "PATH=%__PATH%"
