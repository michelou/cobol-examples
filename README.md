...
We execute command [**`setenv.bat`**](setenv.bat) once to setup our development environment; it makes external tools such as [**`git.exe`**][git_cli], [**`make.exe`**][make_cli] and [**`sh.exe`**][sh_cli] directly available from the command prompt.
   ...
   Tool versions:
   cobc 3.3-dev.0, ccbl 11.0.0.84, cobj 1.1.20, java 17.0.19, make 4.4.1,
...
OpenCOBOL Configuration:
  ...
  COB_CONFIG_DIR       ${datarootdir}/opensource-cobol-4j-1.1.20/config
  COB_COPY_DIR         ${datarootdir}/opensource-cobol-4j-1.1.20/copy
  COB_LIBRARY_PATH     ${exec_prefix}/lib/opensource-cobol-4j-1.1.20
  COB_MODULE_EXT
...
make[2]: Leaving directory '/j/opensourcecobol4j-1.1.20/libcobj'
make[2]: Entering directory '/j/opensourcecobol4j-1.1.20'
...
|           libcobj.jar
\---<b>share</b>
    \---<b>opensource-cobol-4j-1.1.20</b>
        +---<b>config</b>
        |       bs2000.conf
        |       cobol2002.conf
        |       cobol85.conf
